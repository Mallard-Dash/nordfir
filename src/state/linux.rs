...
impl Default for LinuxStateCollector {
    fn default() -> Self {
        Self {
            cpu_sample_interval: Duration::from_millis(120),
        }
    }
}
...
impl LinuxStateCollector {
    pub fn collect(&self, node: NodeId) -> Result<NodeSnapshot, String> {
        ...
        let cpu = self
            .cpu_utilization()
            .map(|value| Observation::known(value, "procfs:/proc/stat"));
        let memory =
            memory_utilization().map(|value| Observation::known(value, "procfs:/proc/meminfo"));
    ...
    fn cpu_utilization(&self) -> Result<f32, String> {
        ...
        if total_delta == 0 {
            return Err("CPU counters did not advance".to_owned());
        }
...
fn read_cpu_times() -> Result<CpuTimes, String> {
    ...
    let line = content
        .lines()
        .next()
        .ok_or_else(|| "missing aggregate cpu line".to_owned())?;
...
fn memory_utilization() -> Result<f32, String> {
    ...
    if total == 0.0 {
        return Err("MemTotal was zero".to_owned());
    }
    Ok(((total - available).max(0.0) / total) * 100.0)
}

fn parse_kib(value: &str) -> Option<f32> {
...
fn uptime_seconds() -> Result<u64, String> {
    ...
    let seconds = content
        .split_whitespace()
        .next()
        .ok_or_else(|| "uptime value missing".to_owned())?;
    Ok(seconds
        .parse::<f64>()
        .map_err(|e| format!("parse uptime: {e}"))? as u64)
}
...
fn load_average_1m() -> Result<f32, String> {
    ...
    content
        .split_whitespace()
        .next()
        .ok_or_else(|| "load average missing".to_owned())?
        .parse::<f32>()
        .map_err(|e| format!("parse load average: {e}"))
}
