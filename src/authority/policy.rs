...
use super::{AuthenticationFactor, AuthorizationDecision, AuthorizationRequest, AuthorityGate, Capability};
...
impl AuthorityPolicy {
    pub fn observe_only() -> Self {
        Self { granted: BTreeSet::from([Capability::Observe]), require_step_up_for: BTreeSet::new() }
    }
...
impl PolicyAuthorityGate {
    pub fn new(policy: AuthorityPolicy) -> Self { Self { policy } }
}
...
impl AuthorityGate for PolicyAuthorityGate {
    fn authorize(&self, request: &AuthorizationRequest) -> AuthorizationDecision {
        if !self.policy.granted.contains(&request.capability) {
            return AuthorizationDecision::Deny { reason: format!("capability {:?} is not granted", request.capability) };
        }
        if self.policy.require_step_up_for.contains(&request.capability) {
            return AuthorizationDecision::Challenge { required_factors: vec![AuthenticationFactor::MasterSecret] };
        }
