    ...
    fn evaluate(&self, snapshot: &NodeSnapshot, action: &Action) -> GuardVerdict {
        if !matches!(
            ...
            return verdict(
                self.name(),
                GuardOutcome::Allow,
                "Action does not enter REST mode",
            );
        }
            ...
            match &observation.value {
                ...
                ObservationValue::Known(ServiceActivity::Active { reason }) => {
                    return verdict(
                        self.name(),
                        GuardOutcome::Defer,
                        format!("Protected service {} is active: {reason}", expected.0),
                    );
                }
                ...
                | ObservationValue::Unavailable { reason } => {
                    return verdict(
                        ...
                        GuardOutcome::Block,
                        format!(
                            "Protected service {} activity is unknown: {reason}",
                            expected.0
                        ),
                    );
                }
        ...
        verdict(
            self.name(),
            GuardOutcome::Allow,
            "All protected services are idle or no protected services are configured",
        )
    }
}
